...
class ChatProvider extends ChangeNotifier {
  ...
  //initialized textModel

  GenerativeModel? _textModel;

  //initialized imageModel
...
// load the messages from db
  Future<List<Message>> loadMessagesFromDB({required String chatId}) async {
    ...
    await Hive.openBox('${AppStrings.chatMessagesBox}$chatId');

    final messageBox = Hive.box('${AppStrings.chatMessagesBox}$chatId');
...
// delete caht
  Future<void> deletChatMessages({required String chatId}) async {
    ...
    if (!Hive.isBoxOpen('${AppStrings.chatMessagesBox}$chatId')) {
      // open the box
      await Hive.openBox('${AppStrings.chatMessagesBox}$chatId');

      // delete all messages in the box
      await Hive.box('${AppStrings.chatMessagesBox}$chatId').clear();

      // close the box
      await Hive.box('${AppStrings.chatMessagesBox}$chatId').close();
    } else {
      // delete all messages in the box
      await Hive.box('${AppStrings.chatMessagesBox}$chatId').clear();
  ...
  Future<void> saveMessagesToDB({
    ...
    final chatHistory = ChatHistory(
      ...
      imageUrls: userMessage.imageUrl,
  ...
  static initHive() async {
    ...
    Hive.init(dir.path);
    await Hive.initFlutter(AppStrings.mboacareDB);
    ...
    if (!Hive.isAdapterRegistered(0)) {
      ...
      await Hive.openBox<ChatHistory>(AppStrings.chatHistoryBox);
    }
    ...
    if (!Hive.isAdapterRegistered(2)) {
      ...
      //open the chat history box
      await Hive.openBox<Settings>(AppStrings.settingsBox);
    }
  }
}
